...
# Vectorized Mandelbrot calculation with optional numba JIT
if NUMBA_AVAILABLE:
    ...
    def compute_mandelbrot_numba(
        ...
        for i in range(height):
            for j in range(width):
                ...
                z = 0.0j
                count = 0
                while abs(z) <= 2.0 and count < max_iter:
                    z = z * z + c
                    count += 1
                mandelbrot_set[i, j] = count
        return mandelbrot_set

    compute_mandelbrot = compute_mandelbrot_numba
    ...
    compute_mandelbrot = compute_mandelbrot_numpy


# Initial view
current_view = (re_min, re_max, im_min, im_max)

fig, ax = plt.subplots(figsize=(8, 6))  # Standardized window size
mandelbrot_img = None
rect_patch = None
press_event = None
